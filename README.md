...
4단계까지가 모션을 정책으로 바꾸는 파이프라인이고, 5단계는 그 결과물을 다른
시뮬레이터에서 검증하는 단계입니다.
...
## 5단계. 다른 시뮬레이터에서 돌려 보기 (sim-to-sim)

4단계까지가 모션을 정책으로 바꾸는 파이프라인이고, 여기서부터는 그 결과물을
검증하는 단계입니다. 학습한 시뮬레이터 안에서만 잘 도는 정책은 접촉 모델이
조금만 달라져도 무너집니다. 두 번째 시뮬레이터에서 그대로 돌려 보는 것이
그 사실을 확인하는 가장 싼 방법입니다.

정책을 ONNX로 내보내 MuJoCo에서 `mujoco_sim_ros2` + `ros2_control`로 그대로
돌립니다. 재학습도, 미세조정도, 게인 재조정도 하지 않습니다. 도메인 랜덤화는
양쪽 다 끕니다.

### 롤아웃 유효성부터 거릅니다

**17편 중 9편은 물리적으로 유효하지 않아 지표에서 제외합니다.**
판정은 골반 높이 하나만 봅니다 (`src/sim2sim_validity.py`). G1은 선 자세에서
골반이 0.79 m 근처이므로, 시작 골반 높이가 0.60~0.95 m 밖이면 롤아웃이
시작부터 틀린 것이고, 골반이 -1.0~1.5 m를 벗어나면 넘어진 것이 아니라 솔버가
터진 것입니다.

```
정상 완주        6편
넘어짐(정상)     2편
초기상태 이상    4편   시작 골반 z 가 -0.43 ~ 0.14 m. 땅에 붙거나 땅속에서 시작
수치 발산        5편   53~190초까지 정상으로 가다가 골반이 -31 m 또는 +42 m 로
```

이것을 정책 실패로 읽으면 정책을 실제보다 나쁘게 보고하게 됩니다.

### 유효한 8편의 결과

| 시퀀스 | Eg-mpbpe(mm) Isaac→MuJoCo | E-mpbpe(mm) Isaac→MuJoCo | MPJPE(rad) Isaac→MuJoCo | 배수(E-mpbpe) | 생존 |
| --- | --- | --- | --- | --- | --- |
| aiming1_subject1 | 89 → 5430 | 35 → 246 | 0.080 → 0.233 | 6.9× | 버팀 (100%) |
| ~~dance2_subject3~~ | — | — | — | — | **무효 (초기상태 이상)** |
| ~~jumps1_subject1~~ | — | — | — | — | **무효 (초기상태 이상)** |
| ~~obstacles2_subject1~~ | — | — | — | — | **무효 (수치 발산)** |
| obstacles3_subject3 | 162 → 3516 | 54 → 249 | 0.101 → 0.244 | 4.6× | 넘어짐 (61%) |
| run2_subject4 | 178 → 6772 | 47 → 248 | 0.111 → 0.263 | 5.3× | 넘어짐 (11%) |
| walk1_subject1 | 80 → 6179 | 34 → 210 | 0.066 → 0.178 | 6.2× | 버팀 (100%) |
| walk1_subject2 | 79 → 7917 | 34 → 228 | 0.066 → 0.194 | 6.6× | 버팀 (100%) |
| ~~walk1_subject5~~ | — | — | — | — | **무효 (수치 발산)** |
| walk2_subject1 | 114 → 2671 | 43 → 225 | 0.101 → 0.284 | 5.2× | 버팀 (100%) |
| ~~walk2_subject3~~ | — | — | — | — | **무효 (수치 발산)** |
| walk2_subject4 | 90 → 9814 | 40 → 226 | 0.085 → 0.217 | 5.6× | 버팀 (100%) |
| ~~walk3_subject1~~ | — | — | — | — | **무효 (수치 발산)** |
| ~~walk3_subject2~~ | — | — | — | — | **무효 (초기상태 이상)** |
| ~~walk3_subject4~~ | — | — | — | — | **무효 (수치 발산)** |
| walk3_subject5 | 85 → 3324 | 36 → 277 | 0.082 → 0.193 | 7.8× | 버팀 (100%) |
| ~~walk4_subject1~~ | — | — | — | — | **무효 (초기상태 이상)** |

**물리적으로 무효한 롤아웃 9개는 제외한다** (시작 자세가 땅속이거나 골반이 수십 m 로 튄 경우).
판정은 `src/sim2sim_validity.py`, 골반 높이만 본다.

표본 8개 · 평균
- Eg-mpbpe  110 → 5703 mm  (52.1×)
- E-mpbpe   41 → 239 mm  (5.9×)
- MPJPE     0.087 → 0.226 rad (2.6×)
- MuJoCo 에서 넘어짐  2/8 (유효분만)

여기서는 E_g-mpbpe가 아니라 E_mpbpe를 봐야 합니다. 전역 오차는 4분짜리 클립
동안 쌓이는 루트 드리프트가 지배하고, 앵커 정렬 후 오차가 자세를 더 정직하게
보여 줍니다.

남은 격차의 원인은 아직 특정하지 못했습니다. 학습 설정에 액추에이터 지연,
토크 한계, 관절 마찰 모델이 없고 두 엔진의 접촉 솔버도 다릅니다. 둘 다 후보로
열려 있습니다.
...
| `scripts/make_sim2sim.sh` | MuJoCo 추적 카메라 렌더 + Isaac 과 나란히 합성 |
| `scripts/write_sim2sim_notes.py` | sim-to-sim 영상의 유튜브 제목·설명문 |
...
| MuJoCo 3.10 / mujoco_sim_ros2 | 5단계 sim-to-sim 실행과 오프스크린 렌더 |
