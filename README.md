...
인덱스 1), 학생 관측은 160차원입니다. 교사는 전부 [512, 256, 128]인데 학생은
[1024, 512, 256]으로 키웠습니다. BumbleBee 논문이 3층 MLP 용량으로는 여러
전문가를 담지 못해 Transformer로 바꿨다고 적어두었기 때문입니다.
...
학습이 돌아가는 것까지 확인했습니다. 환경 4096개가 RTX 5080 16GB에 올라가고,
반복당 0.05초, 20000회에 17분입니다. 손실은 4.44에서 시작해 7000회 부근에서
0.93입니다.

아직 평가하지 않았습니다. 손실이 내려간다는 것은 교사를 흉내 내는 정도가
나아진다는 뜻이지, 로봇이 넘어지지 않고 14개를 완주한다는 뜻이 아닙니다.
개별 교사 대비 완주율과 추적 오차가 얼마나 떨어지는지, 그리고 학습에 쓰지
않은 모션을 따라가는지는 평가 결과로 따로 적겠습니다.
