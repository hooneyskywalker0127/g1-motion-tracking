...
두 기준이 서로 다른 것을 잰다는 것도 그대로 나옵니다.

| 시퀀스 | BeyondMimic | PolySim |
| --- | --- | --- |
| jumps1_subject1 | 0.98 / 0.97 | 0.05 / 0.14 |
| run2_subject4 | 0.83 / 0.73 | 0.00 / 0.00 |

점프와 달리기입니다. 국소 추적은 끊기지 않는데 전역으로는 0.5 m 넘게 밀립니다.
기준 하나만 쓰면 안 되는 이유입니다.

17개 중 셋은 양쪽 다 0입니다. `obstacles2_subject1`은 8.6%만 살아남는 미수렴
정책이고, `walk3_subject1`과 `walk3_subject4`는 77~87%까지 가다가 막판에
걸립니다. 같은 LAFAN1·G1·BeyondMimic으로 Retargeting Matters가 96~100%를
받으므로, 이 셋은 전이 문제가 아니라 종료 조건과 모션 선별의 문제입니다.
