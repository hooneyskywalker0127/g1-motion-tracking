...
오차 단위는 mm이고 완주가 없는 셋은 정의되지 않습니다.
...
두 기준이 서로 다른 것을 잰다는 것이 `jumps1_subject1`과 `run2_subject4`에서
그대로 나옵니다. 점프와 달리기라 국소 추적은 끊기지 않는데 전역으로는 0.5 m 넘게
밀립니다. 기준 하나만 쓰면 안 되는 이유입니다.
