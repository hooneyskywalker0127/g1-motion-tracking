...
# 이름이 kobe_order_ground 라 *_kobe_level1 패턴에 안 걸렸다. 그래서 앞선 실행이
# 관절 순서와 부유 레퍼런스를 고치기 전 런을 집었다. 고친 런을 이름으로 직접 집는다.
RUN=${RUN_OVERRIDE:-2026-09-14_09-15-07_kobe_order_ground}
...
(cd "$WBT" && OMP_NUM_THREADS=1 timeout 900 "$PY" scripts/rsl_rl/play.py --task=Tracking-Flat-G1-v0 \
...
(cd "$WBT" && OMP_NUM_THREADS=1 timeout 3600 "$PY" scripts/rsl_rl/eval_dump.py --task=Tracking-Flat-G1-v0 \
